...
(function (root, factory) {
    ...
    // step 추정 키워드
    // 주의: '총 N원'(합계 금액)은 CHECKOUT 신호에서 제외 — 장바구니 확인/담기 완료 응답이
    // "총 2,000원이에요" 처럼 합계를 말하는데, 이를 결제 단계로 오판해 결제 화면으로 튀던 문제.
    // 실제 결제는 '결제/주문할게/주문 확정/마무리' 등 명시적 의도 키워드로만 추정한다.
    const STEP_CHECKOUT_PATTERN = /(결제|마무리|주문할게|주문 확정|결제 ?화면)/;
