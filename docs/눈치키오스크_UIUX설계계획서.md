...
> **디바이스 해상도**: 1080 x 1920 px (세로형 터치 키오스크)  
...
**레이아웃**: 1080x1920, 상단 80px, 좌우 40px, 푸터 100px, 터치 80px+, 버튼 80px r16, 카드 r20
...
- `normal` → N01 / `avatar` → A01

---

### N01 — 매장/포장 (`flowN/N01-dine.html`)

**요구사항**: 1-1

- "어디서 드실 건가요?" + 2개 카드(440x400): 매장 / 포장
- 선택 → `--color-border-active` + 체크 → 0.5초 후 N02
...
```
S00 시작 → S01 모드 선택
              ├─ [일반] → N01 → N02 ←→ N03/N04/N05 → P01 → P02 → P03/P04 → P05/P06
...
| N01 | 매장/포장 | `flowN/N01-dine.html` | 1-1 |
...
| 0-4 | 모드 분기 | S01→N01/A01 | navigateTo |
| 1-1~1-5 | 일반 UI 주문 | N01~N04 | 메뉴/옵션/장바구니 |
...
*문서 버전: v2.0 | 최종 수정: 2026.04.15 | 기술 스택: HTML5 + CSS3 + jQuery 3.7.0 + Jinja2 + WebSocket | 디바이스: 1080x1920px 세로형 키오스크*
